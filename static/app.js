          ...
          <div class="article-card d-flex align-items-center m-2 glassy" id="article">
            <div class="row gap-2">
                <div class="d-flex article-image col-lg-3 justify-content-end"><img class="rounded-circle"
                         src="${article.image_url}" alt="${article.title}" /></div>
    ...
    document
      ...
      .addEventListener("keyup", async () => {
        ...
        let response = await fetch(
          `/articles/search?query=${encodeURIComponent(query)}`
        );
        let articles = await response.json();
        if (response.ok) {
          let search_html = "";
                    ...
                    <div class="row gap-2">
                        ...
                        <div class="d-flex flex-column gap-2 col justify-content-start">
                            ...
                            <div class="d-flex justify-content-between">
                                <span style="font-size: 10pt;">Published ${formated}</span>
                                <span class="gap-3">
                                    <button class="btn btn-primary read-btn-inside" data-id="${
                                      article.id
                                    }">Read More</button>
                                </span>
                                </span>
          ...
          document.querySelectorAll(".read-btn-inside").forEach((button) => {
            button.addEventListener("click", async () => {
              ...
              document.getElementById("article-content").innerText =
                ...
                const searchModal = bootstrap.Modal.getInstance(document.getElementById("searchModal")).hide();
                if(searchModal){
                  searchModal.hide();
                }
                new bootstrap.Modal(
